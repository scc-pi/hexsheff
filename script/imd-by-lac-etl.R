# HEADER ---------------------
# Description:  Prepare some data for hex map testing
#               i.e. IMD Score by LAC
# Author: Laurie Platt
...
# IMD score by LAC
df_imd_score_by_lac <- read_excel(str_c(data_folder, "/", sli_areas_file), 
                           sheet = "ASC") %>% 
  rename(asc_locality = area) %>% 
  select(asc_locality, `Index of Multiple Deprivation 2019 (IMD) Score`) %>% 
...
# Save as .rds file
write_rds(df_imd_score_by_lac, file = "data/df_imd_score_by_lac.rds")